...
const CreateTask = () => {
  ...
  const handleRegisterUser = async (e:any):Promise<void> =>{
    ...
    try {
      ...
      const data:any = await fetch("http://localhost:4000/api/task/create",init).then((res) => res.json()).then((data:any) => data)
      if(data?.state){
        setGuests("")
        setTeacher("")
        setSubject("")
    ...
    <AdminLayout>
      <div className="min-h-screen w-full bg-sky-600">
        ...
        <form onSubmit={handleRegisterUser} className=" mx-4 md:mx-auto md:w-1/2">
            <legend className="text-center text-4xl text-white font-black mb-10">Create a task</legend>
