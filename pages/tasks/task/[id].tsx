...
const TaskTarget = () => {
  ...
  const [studentTsk, setStudentTsk] = useState<CompletedTask>({ _id: "" })
  ...
  const handleSendTask = async (e: any) => {
    ...
    if (res) {
      const studentTaskUpdated = { ...studentTsk, file: tsk }
      setStudentTsk(studentTaskUpdated)
    }
  }
  useEffect(() => {
    const call = async () => {
  ...
  useEffect(() => {
    if (task?.studentsCompletedTasks) {
      // getting the student sent task to show the qualification and the task sent
      const studentTk: CompletedTask =
        task?.studentsCompletedTasks.find(
          (tsk: CompletedTask) => tsk?.student == auth._id,
        ) ?? {}
      if (
        studentTk?.qualification != undefined &&
        studentTk?.qualification >= 0
      ) {
            ...
            <div className="flex flex-col md:flex-row pt-1 ">
              ...
              <div className="md:w-1/3 px-5 flex flex-col justify-center items-center">
                ...
                >
                  {studentTsk?.file?.length ? (
                    <div>
                      <p className="w-full p-1 rounded-md bg-white">
                        {studentTsk?.file}
                      </p>
                      {studentTsk?.qualification != undefined &&
                      studentTsk?.qualification >= 0 ? (
                        <p
                          className={`w-full p-1 my-1 rounded-md text-white font-bold ${
                            qualification != undefined && qualification >= 6
                              ? "bg-green-500"
                              : "bg-red-500"
                          }`}
                        >
                    ...
                    <>
                      ...
                      <input
                        className="w-full px-2 py-1 bg-sky-400 text-white font-bold rounded-md cursor-pointer hover:bg-sky-500 transition-colors my-5"
                        type="submit"
                        value="Send Task"
                      />
                    </>
